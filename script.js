...
function addCustomToCart() {
    ...
    if (!file && !description) {
        alert("Please upload an image or describe what you want.");
        return;
    }
    ...
    cart.push({
        ...
        description: description || "Custom print"
    });
...
function requestOrder() {
    ...
    let message = "C4Printz Order Request%0A%0A";
    ...
    cart.forEach((item, index) => {
        ...
        if (item.description) {
            message += `Details: ${item.description}%0A`;
        }

        if (item.fileName) {
            message += `Image: ${item.fileName}%0A`;
        }
    ...
    message += "Please contact me about this order.";

    window.location.href =
        "mailto:c4rocks@icloud.com?subject=C4Printz%20Order&body=" + message;
}
